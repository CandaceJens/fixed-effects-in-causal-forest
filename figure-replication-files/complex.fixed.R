...
data.first <- read.csv(paste(project.dir, "MCresults/complex.fixed.all.csv", sep="/"), 
	header = TRUE)
...
data.complex.fixed.1 <- data.first %>% select(at.reg.no.fe, at.reg.fe,
		base.ate, cluster.ate, ols.fe.ate, lasso.fe.ate, mean.ate, demeaned.ate, 
		part.demeaned.ate) 

data.complex.fixed.1 <- (data.complex.fixed.1 - .5) / .5

		
data.complex.fixed.1.melted <- data.complex.fixed.1 %>% 
				gather(data.complex.fixed.1)
...
Fig2PanelL <- data.complex.fixed.1.melted %>%
 	mutate(data.complex.fixed.1 = fct_relevel(data.complex.fixed.1, c("at.reg.no.fe", "at.reg.fe",
		"base.ate", "cluster.ate", "ols.fe.ate", "lasso.fe.ate", "mean.ate", "demeaned.ate", 
		"part.demeaned.ate"))) %>%
	ggplot(aes(y = data.complex.fixed.1, x = value)) + 
...
###Save:
ggsave(Fig2PanelL, filename = paste(project.dir, "figures/Fig2PanelL.pdf", sep="/"),
       width = 6.33, height = 6, units = "in")
